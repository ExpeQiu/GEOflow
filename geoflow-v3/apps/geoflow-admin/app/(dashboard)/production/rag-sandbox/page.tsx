...
type Hit = { chunk_id: number; content: string; score: number };
...
export default function RagSandboxPage() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setError("");
    try {
      const res = await apiPost<{ hits: Hit[] }>("/api/admin/knowledge-bases/rag-sandbox", t, {
  ...
  return (
    <div>
      <HubHeader title="RAG 沙箱" subtitle="测试知识库召回" />
      ...
      <form onSubmit={onSubmit} className="mb-6 space-y-3 rounded-lg bg-white p-4 shadow-sm ring-1 ring-gray-200">
        <select className="w-full rounded-md border px-3 py-2 text-sm" value={kbId} onChange={(e) => setKbId(Number(e.target.value))}>
          {kbs.map((kb) => (
            <option key={kb.id} value={kb.id}>{kb.name}</option>
          ))}
        </select>
        <input className="w-full rounded-md border px-3 py-2 text-sm" placeholder="输入检索问题" value={query} onChange={(e) => setQuery(e.target.value)} />
      ...
      <ul className="space-y-3">
        {hits.map((h) => (
          <li key={h.chunk_id} className="rounded-lg bg-white p-4 text-sm shadow-sm ring-1 ring-gray-200">
            <div className="mb-1 text-xs text-gray-500">chunk #{h.chunk_id} · score {h.score.toFixed(2)}</div>
