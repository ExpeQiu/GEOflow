...
**技术品牌模式**（`GEOFLOW_TECH_BRAND_MODE=true`）：GEOFlow 作为 GEO 后台引擎，Wiki 对外展现由 **Gweb** 承接；详见 [gweb-integration.md](./gweb-integration.md)。
...
技术 IP 资产 → Wiki Task → GEO 门禁 → 审核
        ↓
distribution_only → GwebWikiPublisher → Gweb MDX
        ↓
...
- **技术品牌模式**：Gweb 为 Wiki 唯一对外出口，Wiki Task 禁止本站发布
