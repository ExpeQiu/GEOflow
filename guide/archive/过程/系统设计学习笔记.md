...
**LangGraph 观测**：Production Hub（`admin.production`）展示 Content Agent 侧车健康与 `content_agent_requests` 统计，不直接触发编排；语义切片在 `GEOFLOW_CONTENT_AGENT_BACKEND=external` 时走侧车 `semantic_chunk` workflow。详见 `guide/langgraph-orchestration.md`。
