...
> **专题代号：** `PROBE-TRUTH`  
> **创建：** 2026-07-14  
> **状态：** M0–M4 主栈已落地（口径分离 / daily_limit / Rank v2 / Citation L1 / 金标表）；月度人工金标仍运营中。B 轨默认答文抽 URL（L1），`CITATION_WEB_SEARCH=true` 时 Kimi 尝试 `$web_search`，失败回退抽 URL。  
> **前置：** [真实闭环 P0](./archive/历史指导/真实闭环P0.md)（Open-API 闭环已通）  
...
| 想上 Playwright 爬登录态助手 | **已落地为辅轨**：`run_cend_probe_scan` + 持久 Profile；不进 daily 主链路，硬限额 ≤20 题/次 |
