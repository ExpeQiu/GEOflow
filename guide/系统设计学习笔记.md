...
相关文档：`README.md`（运行结构）、`docs/geo-strategy-architecture.md`（GEO 评估）、`docs/distribution/multi-platform-distribution-plan.md`（分发方案）。
...
| 层 | 回答的问题 | 典型模块 / 入口 |
|----|------------|-----------------|
| **L1 策略洞察与数据分析** | 内容好不好？AI 有没有引用？该怎么写？ | `/admin/analytics`、`InsightTemplate`、`GeoEval` 诊断、`geo:market-scan` |
