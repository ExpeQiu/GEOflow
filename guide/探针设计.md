...
engine          # corpus / llm / api
sentiment       # 情感极性
competitor_mentions  # 竞品提及列表
```
...
| 区域 | 功能 | 对应组件 |
|------|------|----------|
| 概览条 | 问题总数、品牌类/产品类计数、模板数、跳转场景图谱 | `QuestionBankPanel` 顶部 |
| 问题库 Tab | 手工添加/编辑/删除探针问题 | 表单 + `QuestionTable` |
| 问题模板 Tab | 维护 pattern、一键生成问题 | 模板列表 + `generate` |
| 最近扫描 | 最近 5 次 run 及探针数，链到详情页 | `/strategy/monitor/runs/[id]` |

**与相邻页面的分工：**

- **场景图谱**（`/strategy/scene-graph`）：定义 Persona → Scene → Intent，计算 `gap_rate`
- **问题库**（本页）：把意图落成可执行的 Query 文本
- **数据采集**（`/strategy/collection`）：触发全量扫描，消费问题库
