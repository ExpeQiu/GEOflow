...
return [
    ...
    // 技术品牌模式：关闭公网前台，仅保留后台与 API（Gweb 为 Wiki 展现层）
    'public_site_enabled' => filter_var(env('GEOFLOW_PUBLIC_SITE_ENABLED', true), FILTER_VALIDATE_BOOLEAN),
    'tech_brand_mode' => filter_var(env('GEOFLOW_TECH_BRAND_MODE', false), FILTER_VALIDATE_BOOLEAN),

    // Gweb Wiki 同步（gweb_wiki 分发渠道默认读取此处）
    'gweb' => [
        'base_url' => rtrim(trim((string) env('GWEB_BASE_URL', '')), '/'),
        'sync_secret' => (string) env('GWEB_REVALIDATE_SECRET', env('GWEB_SYNC_SECRET', '')),
        'sync_enabled' => filter_var(env('GWEB_SYNC_ENABLED', false), FILTER_VALIDATE_BOOLEAN),
        'timeout_seconds' => max(5, (int) env('GWEB_SYNC_TIMEOUT_SECONDS', 30)),
        'analytics_url' => trim((string) env('GWEB_ANALYTICS_URL', '')),
    ],
