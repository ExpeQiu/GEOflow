...
@php
    ...
    $contentFormat = (string) old('content_format', (string) ($taskForm['content_format'] ?? 'article'));
    $wikiPageType = (string) old('wiki_page_type', (string) ($taskForm['wiki_page_type'] ?? 'concept'));
    ...
    <div class="px-4 sm:px-0">
        ...
        <div data-task-form-shell class="w-full">
            ...
            <form method="POST" action="{{ $isEdit ? route('admin.tasks.update', ['taskId' => $taskId]) : route('admin.tasks.store') }}" class="grid grid-cols-1 gap-6 xl:grid-cols-12">
                ...
                <div class="bg-white shadow rounded-lg xl:col-span-12" data-wiki-task-panel>
                    <div class="px-6 py-4 border-b border-gray-200">
                        <h3 class="text-lg font-medium text-gray-900">{{ $t('wiki_task.section_title') }}</h3>
                        <p class="mt-1 text-sm text-gray-600">{{ $t('wiki_task.section_desc') }}</p>
                    </div>
                    <div class="px-6 py-4 grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            <label class="block text-sm font-medium text-gray-700">{{ $t('wiki_task.field_content_format') }}</label>
                            <select name="content_format" id="content_format" data-wiki-format-select class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                <option value="article" @selected($contentFormat === 'article')>{{ $t('wiki_task.format_article') }}</option>
                                <option value="wiki_mdx" @selected($contentFormat === 'wiki_mdx')>{{ $t('wiki_task.format_wiki') }}</option>
                            </select>
                        </div>
                        <div data-wiki-only-fields @class(['hidden' => $contentFormat !== 'wiki_mdx'])>
                            <label class="block text-sm font-medium text-gray-700">{{ $t('wiki_task.field_wiki_type') }}</label>
                            <select name="wiki_page_type" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                @foreach (($formOptions['wikiPageTypes'] ?? ['concept']) as $type)
                                    <option value="{{ $type }}" @selected($wikiPageType === $type)>{{ $type }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div data-wiki-only-fields @class(['hidden' => $contentFormat !== 'wiki_mdx'])>
                            <label class="block text-sm font-medium text-gray-700">{{ $t('wiki_task.field_tech_ip') }}</label>
                            <select name="tech_ip_asset_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                <option value="">—</option>
                                @foreach (($formOptions['techIpAssets'] ?? []) as $asset)
                                    <option value="{{ $asset['id'] }}" @selected((string) old('tech_ip_asset_id', (string) ($taskForm['tech_ip_asset_id'] ?? '')) === (string) $asset['id'])>
                                        {{ $asset['ip_name'] }} ({{ $asset['status'] }})
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="md:col-span-3 text-sm text-indigo-700 bg-indigo-50 rounded-md px-4 py-3 @if($contentFormat !== 'wiki_mdx') hidden @endif" data-wiki-hint>
                            {{ $t('wiki_task.hint_gweb_only') }}
                        </div>
                    </div>
                </div>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function syncWikiTaskMode() {
                const formatSelect = document.getElementById('content_format');
                if (!formatSelect) {
                    return;
                }
                const isWiki = formatSelect.value === 'wiki_mdx';
                document.querySelectorAll('[data-wiki-only-fields]').forEach((el) => el.classList.toggle('hidden', !isWiki));
                const hint = document.querySelector('[data-wiki-hint]');
                if (hint) {
                    hint.classList.toggle('hidden', !isWiki);
                }
                if (isWiki) {
                    const distributionOnly = document.querySelector('input[name="publish_scope"][value="distribution_only"]');
                    if (distributionOnly) {
                        distributionOnly.checked = true;
                    }
                    document.querySelectorAll('input[name="publish_scope"]').forEach((radio) => {
                        if (radio.value !== 'distribution_only') {
                            radio.disabled = true;
                        }
                    });
                    document.getElementById('auto_keywords')?.setAttribute('disabled', 'disabled');
                    document.getElementById('auto_description')?.setAttribute('disabled', 'disabled');
                } else {
                    document.querySelectorAll('input[name="publish_scope"]').forEach((radio) => {
                        radio.disabled = false;
                    });
                    document.getElementById('auto_keywords')?.removeAttribute('disabled');
                    document.getElementById('auto_description')?.removeAttribute('disabled');
                }
            ...
            document.getElementById('content_format')?.addEventListener('change', syncWikiTaskMode);
