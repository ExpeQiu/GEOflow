...
@php
    $channelType = old('channel_type', config('geoflow.tech_brand_mode') ? 'gweb_wiki' : 'geoflow_agent');
                ...
                <form method="POST" action="{{ route('admin.distribution.store') }}" class="space-y-6">
                    ...
                    <fieldset class="rounded-lg border border-gray-200 bg-gray-50 p-4">
                        ...
                        <div class="mt-4 grid grid-cols-1 gap-3 lg:grid-cols-2 xl:grid-cols-4">
                            ...
                            <label class="flex cursor-pointer gap-3 rounded-md border border-gray-200 bg-white p-4 hover:border-blue-300">
                                <input type="radio" name="channel_type" value="gweb_wiki" class="mt-1 text-blue-600 focus:ring-blue-500" @checked($channelType === 'gweb_wiki')>
                                <span>
                                    <span class="block text-sm font-semibold text-gray-900">{{ __('admin.distribution.channel_type.gweb_wiki') }}</span>
                                    <span class="mt-1 block text-sm text-gray-600">{{ __('admin.distribution.channel_type.gweb_wiki_desc') }}</span>
                                </span>
                            </label>
                            <label class="flex cursor-pointer gap-3 rounded-md border border-gray-200 bg-white p-4 hover:border-blue-300">
                    ...
                    <div data-channel-type-panel="gweb_wiki" @class(['rounded-lg border border-teal-100 bg-teal-50 p-5', 'hidden' => $channelType !== 'gweb_wiki'])>
                        <div class="mb-5">
                            <h2 class="text-lg font-medium text-gray-900">{{ __('admin.distribution.gweb.section_title') }}</h2>
                            <p class="mt-1 text-sm leading-6 text-gray-600">{{ __('admin.distribution.gweb.section_desc') }}</p>
                        </div>
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            <div>
                                <label for="gweb_sync_secret" class="block text-sm font-medium text-gray-700">{{ __('admin.distribution.gweb.sync_secret') }} *</label>
                                <input id="gweb_sync_secret" name="gweb_sync_secret" type="password" value="{{ old('gweb_sync_secret') }}" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500" autocomplete="new-password">
                                <p class="mt-1 text-xs text-gray-500">{{ __('admin.distribution.gweb.sync_secret_help') }}</p>
                            </div>
                            <div>
                                <label for="gweb_timeout_seconds" class="block text-sm font-medium text-gray-700">{{ __('admin.distribution.gweb.timeout_seconds') }}</label>
                                <input id="gweb_timeout_seconds" name="gweb_timeout_seconds" type="number" min="5" max="120" value="{{ old('gweb_timeout_seconds', config('geoflow.gweb.timeout_seconds', 30)) }}" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            </div>
                        </div>
                        <p class="mt-4 text-xs text-teal-800">{{ __('admin.distribution.gweb.endpoint_hint') }}</p>
                    </div>
