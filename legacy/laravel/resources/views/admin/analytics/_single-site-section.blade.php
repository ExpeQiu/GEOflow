...
<section class="mb-8" data-analytics-single-site-section>
    @if (config('geoflow.tech_brand_mode'))
        <div class="mb-4 rounded-md border border-indigo-200 bg-indigo-50 px-4 py-3 text-sm text-indigo-800">
            {{ __('admin.analytics.tech_brand_pv_note') }}
            @if (!empty(config('geoflow.gweb.analytics_url')))
                <a href="{{ config('geoflow.gweb.analytics_url') }}" target="_blank" rel="noopener" class="ml-2 font-medium text-indigo-900 underline">{{ __('admin.strategy.gweb_analytics_link') }}</a>
            @endif
        </div>
    @endif
    <div class="mb-5">
