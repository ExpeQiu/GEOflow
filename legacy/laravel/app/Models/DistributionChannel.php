...
class DistributionChannel extends Model
{
    ...
    public function resolvedSiteSettings(): array
    {
        ...
        $rawSiteName = $stored['site_name'] ?? $this->name ?? 'GEOworkflow Target Site';
        $siteName = trim((string) $rawSiteName);

        return [
            'site_name' => $siteName !== '' ? $siteName : 'GEOworkflow Target Site',
            'site_subtitle' => trim((string) ($stored['site_subtitle'] ?? '')),
            'site_description' => trim((string) ($stored['site_description'] ?? '由 GEOworkflow 自动分发和管理的目标站点。')),
            'site_keywords' => trim((string) ($stored['site_keywords'] ?? '')),
            'copyright_info' => trim((string) ($stored['copyright_info'] ?? '© '.date('Y').' '.($siteName !== '' ? $siteName : 'GEOworkflow Target Site'))),
    ...
    public function channelType(): string
    {
        ...
        return in_array($type, ['geoflow_agent', 'wordpress_rest', 'generic_http_api', 'gweb_wiki'], true) ? $type : 'geoflow_agent';
    }
    ...
    public function isGwebWiki(): bool
    {
        return $this->channelType() === 'gweb_wiki';
    }

    /**
     * @return array{base_url:string,sync_secret:string,timeout_seconds:int}
     */
    public function resolvedGwebConfig(): array
    {
        $stored = is_array($this->channel_config) ? $this->channel_config : [];
        $baseUrl = trim((string) ($stored['gweb_base_url'] ?? $this->endpoint_url ?? ''));
        if ($baseUrl === '') {
            $baseUrl = trim((string) config('geoflow.gweb.base_url', ''));
        }
        $secret = trim((string) ($stored['gweb_sync_secret'] ?? ''));
        if ($secret === '') {
            $secret = trim((string) config('geoflow.gweb.sync_secret', ''));
        }

        return [
            'base_url' => rtrim($baseUrl, '/'),
            'sync_secret' => $secret,
            'timeout_seconds' => max(5, min(120, (int) ($stored['gweb_timeout_seconds'] ?? config('geoflow.gweb.timeout_seconds', 30)))),
        ];
    }
