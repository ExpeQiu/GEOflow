...
class GeoCheckAdoptionAlertsCommand extends Command
{
    ...
    public function handle(
        ...
        if ($stats['sample_size'] > 0 && $adoption < $adoptionFloor) {
            ...
            $alerts += $this->markTechIpAssetsNeedsUpdate($adoption, $adoptionFloor);
        }
    ...
    /** 采纳率低于阈值时，将已发布技术 IP 标记为需更新（技术品牌闭环）。 */
    private function markTechIpAssetsNeedsUpdate(float $adoption, float $floor): int
    {
        if (! config('geoflow.tech_brand_mode') || ! Schema::hasTable('tech_ip_assets')) {
            return 0;
        }

        $count = TechIpAsset::query()
            ->where('status', '已发布')
            ->update(['status' => '需更新', 'updated_at' => now()]);

        if ($count > 0) {
            Log::info('tech_brand.adoption_mark_needs_update', [
                'adoption_rate' => $adoption,
                'floor' => $floor,
                'assets_marked' => $count,
            ]);
            $this->info("Marked {$count} tech IP asset(s) as 需更新.");
        }
