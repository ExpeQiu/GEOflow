...
class TaskController extends Controller
{
    ...
    public function create(Request $request): View
    {
        ...
        $taskForm = null;
        if ($request->filled('content_format') || $request->filled('tech_ip_asset_id')) {
            $taskForm = [
                'content_format' => (string) $request->query('content_format', 'wiki_mdx'),
                'wiki_page_type' => (string) $request->query('wiki_page_type', 'concept'),
                'tech_ip_asset_id' => (string) $request->query('tech_ip_asset_id', ''),
                'publish_scope' => (string) $request->query('publish_scope', 'distribution_only'),
            ];
        }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $payload = WikiTaskPolicy::applyDefaults($payload);
        try {
            WikiTaskPolicy::assertValid($payload, $request);
    ...
    public function edit(int $taskId): View|RedirectResponse
    {
        ...
        return view('admin.tasks.form', [
            ...
            'taskForm' => [
                ...
                'content_format' => (string) ($task['content_format'] ?? 'article'),
                'wiki_page_type' => (string) ($task['wiki_page_type'] ?? 'concept'),
                'tech_ip_asset_id' => (string) (($task['tech_ip_asset_id'] ?? '') ?: ''),
    ...
    private function loadTaskFormOptions(): array
    {
        ...
        $distributionChannels = DistributionChannel::query()
            ->select(['id', 'name', 'domain', 'channel_type'])
            ->where('status', 'active')
            ->orderBy('name')
            ->get()
        ...
        if (Schema::hasTable('tech_ip_assets')) {
            $techIpAssets = TechIpAsset::query()
                ->orderByDesc('priority')
                ->orderBy('ip_name')
                ->get(['id', 'ip_id', 'ip_name', 'mind_tag', 'wiki_type', 'wiki_slug', 'status'])
                ->map(static fn (TechIpAsset $row): array => [
                    'id' => (int) $row->id,
                    'ip_id' => (string) $row->ip_id,
                    'ip_name' => (string) $row->ip_name,
                    'mind_tag' => (string) ($row->mind_tag ?? ''),
                    'wiki_type' => (string) ($row->wiki_type ?? ''),
                    'wiki_slug' => (string) ($row->wiki_slug ?? ''),
        ...
        return [
            ...
            'techIpAssets' => $techIpAssets,
            'wikiPageTypes' => TechIpAsset::wikiTypes(),
            'defaultWikiPublishScope' => 'distribution_only',
        ];
    }
    ...
    private function validateTaskForm(Request $request): array
    {
        return $request->validate([
            ...
            'content_format' => ['nullable', 'string', 'in:article,wiki_mdx'],
            'wiki_page_type' => ['nullable', 'string', 'in:concept,compare,guide,glossary,data,thread,topic'],
            'tech_ip_asset_id' => ['nullable', 'integer', 'min:1'],
    ...
    private function buildTaskPayload(Request $request, array $payload): array
    {
        ...
        return [
            ...
            'content_format' => (string) ($payload['content_format'] ?? 'article'),
            'wiki_page_type' => WikiTaskPolicy::isWikiFormat((string) ($payload['content_format'] ?? 'article'))
                ? (string) ($payload['wiki_page_type'] ?? 'concept')
                : null,
            'tech_ip_asset_id' => isset($payload['tech_ip_asset_id']) && (int) $payload['tech_ip_asset_id'] > 0
                ? (int) $payload['tech_ip_asset_id']
                : null,
