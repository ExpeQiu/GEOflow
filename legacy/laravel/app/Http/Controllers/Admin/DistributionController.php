...
class DistributionController extends Controller
{
    ...
    private function validateChannel(Request $request): array
    {
        $payload = $request->validate([
            ...
            'channel_type' => ['nullable', 'string', 'in:geoflow_agent,wordpress_rest,generic_http_api,gweb_wiki'],
            ...
            'gweb_sync_secret' => ['nullable', 'string', 'max:1000'],
            'gweb_timeout_seconds' => ['nullable', 'integer', 'min:5', 'max:120'],
        ...
        if ($payload['channel_type'] === 'gweb_wiki') {
            if ($request->isMethod('post') && ! filled($payload['gweb_sync_secret'] ?? null)) {
                throw ValidationException::withMessages([
                    'gweb_sync_secret' => __('admin.distribution.validation.generic_secret'),
                ]);
            }
        }
    ...
    {
        $defaultName = trim((string) ($payload['name'] ?? 'GEOworkflow Target Site'));
        $defaults = $channel?->resolvedSiteSettings() ?? [
            'site_name' => $defaultName !== '' ? $defaultName : 'GEOworkflow Target Site',
            'site_subtitle' => '',
            'site_description' => '由 GEOworkflow 自动分发和管理的目标站点。',
            'site_keywords' => '',
            'copyright_info' => '© '.date('Y').' '.($defaultName !== '' ? $defaultName : 'GEOworkflow Target Site'),
            'site_logo' => '',
            'site_favicon' => '',
    ...
    private function normalizeChannelConfig(array $payload, ?DistributionChannel $channel = null): array
    {
        ...
        if ($channelType === 'gweb_wiki') {
            $defaults = $channel?->resolvedGwebConfig() ?? (new DistributionChannel)->resolvedGwebConfig();

            return [
                'gweb_base_url' => rtrim(trim((string) ($payload['endpoint_url'] ?? $defaults['base_url'])), '/'),
                'gweb_sync_secret' => trim((string) ($payload['gweb_sync_secret'] ?? $defaults['sync_secret'])),
                'gweb_timeout_seconds' => min(120, max(5, (int) ($payload['gweb_timeout_seconds'] ?? $defaults['timeout_seconds']))),
            ];
        }
