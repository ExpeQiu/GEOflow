...
final class ArticleEvaluationService
{
    public function __construct(
        ...
        private readonly WikiGeoComplianceChecker $wikiGeoComplianceChecker,
    ) {}
    ...
    public function evaluateArticle(int $articleId, ?int $taskRunId = null, ?int $taskId = null): array
    {
        ...
        try {
            ...
            $wikiCompliance = null;
            if ($passed && config('geo_eval.wiki_gate.enabled', true) && $article->isWikiMdx()) {
                $wikiCompliance = $this->wikiGeoComplianceChecker->check($article);
                if (! ($wikiCompliance['passed'] ?? false)) {
                    $passed = false;
                }
            }
            ...
            if (is_array($wikiCompliance)) {
                $mergedMetrics['wiki_compliance'] = $wikiCompliance;
            }

            $status = $passed ? 'passed' : 'failed';
            $reason = $passed ? null : $this->buildFailureReason($metrics, $auditData, $wikiCompliance);
    ...
    /**
     * @param  array<string, mixed>|null  $wikiCompliance
     * @param  array<string, mixed>  $auditData
     */
    private function buildFailureReason(array $metrics, array $auditData, ?array $wikiCompliance = null): string
    {
        $parts = [];
        ...
        if (is_array($wikiCompliance) && ! ($wikiCompliance['passed'] ?? true)) {
            $failedChecks = collect($wikiCompliance['checks'] ?? [])
                ->filter(static fn (array $c): bool => ! ($c['passed'] ?? false))
                ->pluck('key')
                ->implode(',');
            $parts[] = 'wiki_compliance_failed'.($failedChecks !== '' ? ':'.$failedChecks : '');
        }
