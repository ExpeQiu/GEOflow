...
final class WorkerArticlePersistenceService
{
    public function __construct(
        ...
        private readonly WikiArticleMetaBuilder $wikiArticleMetaBuilder,
    ) {}
    ...
    public function persistGeneratedArticle(array $context): array
    {
        ...
        $articleId = DB::transaction(function () use ($task, $titleRow, $author, $category, $keyword, $content, $excerpt, $workflow, $selectedImages): int {
            ...
            $article = Article::query()->create([
                ...
                'excerpt' => $excerpt,
                'content' => $content,
                'content_format' => $task->isWikiMdx() ? 'wiki_mdx' : 'article',
                'wiki_meta' => $task->isWikiMdx()
                    ? $this->wikiArticleMetaBuilder->buildForTask($task, (string) $titleRow->title, $content)
                    : null,
                'tech_ip_asset_id' => (int) ($task->tech_ip_asset_id ?? 0) > 0 ? (int) $task->tech_ip_asset_id : null,
